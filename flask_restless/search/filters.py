...
def from_dictionary(model, custom_operators, dictionary):
    ...
    if 'or' not in d and 'and' not in d and 'not' not in d:
        ...
        if 'field' in dictionary:
            ...
            if operator in ('has', 'any'):
                ...
                argument = from_dictionary(related_model, custom_operators,
                                           argument)
                return FieldFilter(field, operator, argument)
            # HACK: need to deal with the special case of converting dates.
            argument = string_to_datetime(model, fieldname, argument)
            return FieldFilter(field, operator, argument, custom_operators)
    from_dict = partial(from_dictionary, model, custom_operators)
...
def create_filters(model, filters, custom_operators=None):
